...
export default function CountryCard({ country, onClose }) {
  ...
  console.log('CountryCard render - country:', country);

  useEffect(() => {
    console.log('CountryCard useEffect - country changed:', country);
    if (country) {
      console.log('CountryCard - fetching data for:', country.name);
      fetchCountryData(country);
    } else {
      console.log('CountryCard - no country, clearing data');
      setCountryData(null);
    }
  }, [country]);
  ...
  if (!country) {
    console.log('CountryCard - no country, returning null');
    return null;
  }

  console.log('CountryCard - rendering with country:', country.name);

  return (
    <div className="fixed top-6 right-6 w-80 h-[calc(100vh-3rem)] bg-slate-900/95 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-2xl z-50 animate-in slide-in-from-right duration-300 flex flex-col">
      {/* Header */}
      <div className="flex justify-between items-center p-4 border-b border-slate-700/50">
        <div>
          <h2 className="text-xl font-bold text-white">{country.name}</h2>
          <p className="text-slate-300 text-sm">Country Information</p>
        </div>
        <button
          onClick={onClose}
        ...
        {countryData && (
          <div className="space-y-4 text-white">
            ...
            <div className="flex justify-center">
              <div className="relative bg-slate-800/30 rounded-xl p-3 border border-slate-700/50 max-w-fit">
                <img
                  src={countryData.flags?.png}
                  alt={`${countryData.name?.common} flag`}
                  className="w-20 h-14 object-contain rounded-lg border border-slate-600/30 shadow-sm"
                  onError={(e) => {
            ...
            <div className="grid grid-cols-1 gap-3 text-sm">
              <div className="bg-slate-800/30 rounded-lg p-3 border border-slate-700/50">
                ...
                <p className="font-semibold text-white mt-1 text-sm">{countryData.name?.official}</p>
              </div>
              
              <div className="bg-slate-800/30 rounded-lg p-3 border border-slate-700/50">
                <span className="text-slate-300 text-xs uppercase tracking-wide">Capital</span>
                <p className="font-semibold text-white mt-1 text-sm">{countryData.capital?.[0] || 'N/A'}</p>
              </div>
            ...
            <div className="bg-slate-800/30 rounded-lg p-3 border border-slate-700/50">
              ...
              <p className="font-semibold text-white mt-1 text-sm">{countryData.region || 'N/A'}</p>
              {countryData.subregion && (
                <>
                  <span className="text-slate-300 text-xs uppercase tracking-wide mt-3 block">Subregion</span>
                  <p className="font-semibold text-white mt-1 text-sm">{countryData.subregion}</p>
                </>
              )}
            </div>
